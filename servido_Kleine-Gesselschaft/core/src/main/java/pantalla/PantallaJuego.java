...
public class PantallaJuego extends ScreenAdapter implements GameController {
    ...
    private static final Pattern CLOTHING_CHAT_PATTERN =
        Pattern.compile("^user \\(\\d+\\) cambio a ropa\\s+(.+)$", Pattern.CASE_INSENSITIVE);
    ...
    private void aplicarCambioDeRopaDesdeMensaje(Jugador target, String message) {
        if (target == null || message == null) return;
        Matcher matcher = CLOTHING_CHAT_PATTERN.matcher(message.trim());
        if (!matcher.matches()) return;

        String clothingName = matcher.group(1).trim();
        ClothingItem base = ropaCatalogoPorNombre.get(normalizarNombreRopa(clothingName));
        if (base == null) {
            Gdx.app.log("NET", "Ropa remota no reconocida: " + clothingName);
            return;
        }
    ...
    public void show() {
        ...
        if (jugador != null && skinUI != null) {
            ...
            inventario = new Inventario(stageInventario, skinUI, jugador, ci -> {
                if (clienteNetwork != null) {
                    clienteNetwork.sendClothingChange(ci.getDisplayName());
                }
            }); // ← firma correcta
    ...
    public void updateChatMessage(int playerId, String message) {
        ...
        Gdx.app.postRunnable(() -> {
            ...
            if (target != null) {
                aplicarCambioDeRopaDesdeMensaje(target, message);
                if (chat != null) {
